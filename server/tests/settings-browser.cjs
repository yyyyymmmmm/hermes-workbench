...
(async()=>{
 ...
 try{
  ...
  for(const theme of ['classic','minimal','dark','blue','red','pink','fluent','glass']){await page.locator(`input[name=theme][value=${theme}]`).check();assert.equal(await page.locator('html').getAttribute('data-theme'),theme);await page.screenshot({path:`test-results/theme-${theme}.png`,fullPage:true,animations:'disabled'});}
  ...
  assert.deepEqual(errors,[]);console.log('PASS: eight themes, persisted language, English across all pages, motion, desktop/mobile');
