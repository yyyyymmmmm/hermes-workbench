...
(async()=>{
 ...
 try{
  ...
  let nativeDialogs=0;page.on('dialog',d=>{nativeDialogs++;void d.dismiss();});
  await page.route('**/api/runs/*/actions/apply',route=>route.fulfill({status:409,contentType:'application/json',body:JSON.stringify({code:'VERSION_CONFLICT',message:'Task changed'})}),{times:1});
  await page.locator('[data-card-op=apply]').click();await page.locator('[data-operation-card] [role=alert]').waitFor();assert.equal((await (await ctx.request.get(config.origin+'/api/tasks')).json()).tasks.length,0);
  await page.locator('[data-card-op=apply]').click();await page.locator('[data-operation-card] .tag').filter({hasText:'已同步到待办'}).waitFor();
  await page.locator('[data-card-op=undo]').click();assert.equal((await (await ctx.request.get(config.origin+'/api/tasks')).json()).tasks.length,1);
  await page.locator('[data-card-op=cancel]').click();
  await page.locator('[data-project-actions]').click();await page.locator('.action-status').filter({hasText:'已同步到待办'}).waitFor();await page.locator('[data-project-chat-close]').click();assert.equal(await page.locator('.project-chat-dialog').count(),0);
  for(const width of [1440,390]){await page.setViewportSize({width,height:900});if(width===390)await page.locator('.mobile-nav [data-action=toggle-ai]').click();await page.screenshot({path:`test-results/interaction-card-${width}.png`,animations:'disabled'});}await page.setViewportSize({width:1440,height:900});
  await page.locator('[data-chat-action=new]').click();await page.locator('#live-chat-form textarea').fill('Authorization test');await page.locator('#live-chat-form [type=submit]').click();await page.locator('[data-consent=cancel]').click();assert.equal((await (await ctx.request.get(config.origin+'/api/runs')).json()).runs.length,1);
  await page.locator('#live-chat-form [type=submit]').click();await page.locator('[data-consent=allow]').click();await page.waitForFunction(()=>!document.querySelector('[data-consent=allow]'));await page.locator('.chat-output').filter({hasText:'report.md'}).waitFor();assert.equal((await (await ctx.request.get(config.origin+'/api/runs')).json()).runs.length,2);assert.equal(nativeDialogs,0);
