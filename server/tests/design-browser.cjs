...
(async()=>{
 ...
 try{
  ...
  await page.goto(config.origin);await page.locator('.recent-project').waitFor();await page.locator('.overview-health [data-page=health]').click();await page.locator('.device-data').waitFor();assert.equal(await page.locator('[data-device-read=health]').isDisabled(),true);assert.equal(await page.locator('.health-stats').count(),0);await page.locator('.sidebar button[data-page=home]').click();mkdirSync('test-results',{recursive:true});
