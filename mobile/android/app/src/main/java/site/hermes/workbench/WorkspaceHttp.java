...
final class WorkspaceHttp {
 ...
 void request(JSONObject input,Consumer<JSONObject> reply){
  String requestId=input.optString("id","");
  if(input.optBoolean("cancel",false)){Call call=streams.remove(requestId);if(call!=null)call.cancel();return;}
  ...
  executor.execute(()->{try{
   ...
   boolean streaming=input.optBoolean("stream",false);
   if(streaming&&(!method.equals("GET")||!path.getRawPath().matches("/api/runs/[a-fA-F0-9-]{36}/events")))throw new Exception();
   ...
   Call call=client.newCall(request.build());if(streaming){if(streams.size()>=2)throw new Exception();streams.put(requestId,call);}
   ...
   if(streaming){
    if(status!=200||!response.header("Content-Type","").startsWith("text/event-stream"))throw new Exception();
    reply.accept(new JSONObject().put("stream",true).put("status",status));
    try(InputStream data=response.body().byteStream()){byte[] chunk=new byte[8192];int n;while((n=data.read(chunk))!=-1)reply.accept(new JSONObject().put("stream",true).put("body",Base64.encodeToString(chunk,0,n,Base64.NO_WRAP)));}
    reply.accept(new JSONObject().put("stream",true).put("done",true));return;
   }
  ...
  }catch(Exception e){reply.accept(error("CONNECTION_FAILED"));}finally{streams.remove(requestId);count.decrementAndGet();}});
 }
