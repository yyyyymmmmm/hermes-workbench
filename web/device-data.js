...
window.HermesDeviceData=(()=>{
 ...
 function calendarPanel(){
  ...
  return `<section class="device-data">${controls('calendar')}<p>${t('只读未来七天，最多显示100条；尚未导入待办或双向同步。','Read-only next seven days, up to 100 events; not imported into tasks or synchronized.')}</p>${calendar?`<p>${e(calendar.source)}</p>${rows||`<p>${t('此时间范围没有可读取日程。','No readable events in this range.')}</p>`}`:''}</section>`;
 }
