...
window.HermesDeviceData=(()=>{
 ...
 const names={steps:['今日步数','Today\'s steps'],weight:['体重','Weight'],restingHeartRate:['静息心率','Resting heart rate'],bodyFat:['体脂率','Body fat'],oxygen:['血氧','Oxygen saturation'],bloodGlucose:['血糖','Blood glucose']};
 ...
 function request(method){return new Promise((resolve,reject)=>{
  const id=crypto.randomUUID(),message={id,method,metrics:[...selected]};
 ...
 document.addEventListener('click',async ev=>{
  const button=ev.target.closest('[data-device-read],[data-device-all],[data-device-clear],[data-device-share]');if(!button||busy||!context?.state.me)return;
  ...
  try{const result=await request(button.dataset.deviceRead==='health'?'health.read':'calendar.read');if(context.state.me?.user.id!==owner)return;if(button.dataset.deviceRead==='health')health=result;else calendar=result;}
