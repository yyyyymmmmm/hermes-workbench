...
window.HermesChat = (() => {
  ...
  function authorize(){return confirm('允许 Hermes 在本会话中按远端权限执行任务？消息和所选附件会发送到你的服务器，可能调用工具并产生费用。授权持续到你撤销或新建会话；远端发来的工具审批仍需单独确认。');}
  async function action(button){
    ...
    try{
      ...
      if(name==='permission'){
        ...
        else granted=authorize();context.render();return;
      }
  ...
  document.addEventListener('submit',async event=>{
    ...
    if(form.id==='live-chat-form'){if(!draft.trim()&&!files.length)return;if(!granted){if(!authorize())return;granted=true;}}
